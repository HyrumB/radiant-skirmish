...
@onready var head_node : Node3D = $Head_origin_point/Rotation_helper
@onready var hitbox : CollisionShape3D = $HitBox
@onready var _original_height = hitbox.shape.height
...
var pants_av   : float = 0
var gloves_av  : float = 0

#-------------damage related var-------------
var damage            : float = 50
var blood_damage      : float = 50
var armor_value       : float =  helmet_av + shirt_av + gloves_av + pants_av
...
func _input(event):
	...
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		head_node.rotate_x(deg_to_rad(event.relative.y * MOUSE_SENSITIVITY * -1))
...
func _physics_process(delta):
	...
	if Input.is_action_just_released("crouch"):
		...
		is_crawling = true
		direction = direction * CRAWL_MULTI
		handle_crawl()
...
func handle_crouch():
	head_node.position = Vector3(0,(-crouch_translate if is_crouched else -.6), 0)
	hitbox.shape.height = _original_height - crouch_translate if is_crouched else _original_height

func handle_crawl():
	head_node.position = Vector3(0,(-crawl_translate if is_crawling else -.6), 0)
	hitbox.shape.height = _original_height - crawl_translate if is_crawling else _original_height
